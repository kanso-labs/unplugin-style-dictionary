...
- **Dependency versions are pinned exactly.** Every `dependencies`,
  `devDependencies` and `optionalDependencies` entry is a bare version, `5.5.2`,
  never `^5.5.2`, `~5.5.2`, `>=5.5.2`, `*`, `5.x` or an `||` union. Renovate
  opens those bumps too. `peerDependencies` are the deliberate exception: they
  state what the consumer's own installed copy must satisfy, so ranges are
